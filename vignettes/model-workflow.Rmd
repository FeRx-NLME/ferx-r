...
### Convergence check before a long run

`ferx_check_init()` runs a short warm-up (default 5 iterations) and returns
an early OFV trace plus a convergence summary. Use it to confirm the gradient
is pointing in the right direction before committing to a full estimation:

```{r check-init, eval = FALSE}
chk <- ferx_check_init(ex$model, ex$data, method = "focei")
chk$summary        # ofv_start, ofv_end, ofv_drop
ferx_plot_trace(chk$fit)   # visual: first few OFV steps
```

If `ofv_drop` is small or negative, the initial values are likely poor — adjust
them in `[initial_values]` or widen the bounds before proceeding.
