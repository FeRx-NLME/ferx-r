...
# if one of the checks were deleted - and two of the three checks exist only
# because the compiler cannot see the shape they hold (a fully qualified
...
fn raise_verbatim(msg: String) -> ! {
    unsafe { Rf_error(b"%s\0".as_ptr().cast(), msg.as_ptr()) }
}
