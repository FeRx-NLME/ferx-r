...
#'       (default 250).}
#'     \item{\code{n_mh_steps}}{(default \code{"auto"}) Metropolis-Hastings
#'       steps per subject per iteration. \code{"auto"} sizes it from the data
#'       as 2.5 x observations per subject per random effect, clamped to
#'       \code{[6, 20]}. Also sizes the componentwise decorrelating kernel that
...
#'       \code{"bayes"}; \code{"bayes"} does not apply the \code{"auto"} rule
#'       and keeps 20 unless you set a number.}
#'     \item{\code{scale_adaptation}}{(default \code{"robbins_monro"}) How the
#'       MH step scales are adapted. \code{"interval"} is the rule used before
#'       ferx-core 0.4.0.}
#'     \item{\code{adapt_interval}}{How often (in iterations) the MH step
#'       scales are adapted under \code{scale_adaptation = "interval"}
#'       (default 50). Also governs the IOV scales under either rule.}
...
#'   theta CL(0.1, 0.001)                  # lower bound only (upper defaults to 1e9)
...
#'     was ignored outright and \strong{no parameter was declared}. The model
#'     still parsed, so every reference to that name silently resolved to
#'     something other than the parameter the user wrote.
#' }
#'
...
ferx_fit <- function(model, data = NULL,
  ...
  # The raw list becomes a `ferx_fit` in one place, shared with the search
  # tools: `ferx_covsearch()` and `ferx_allometry()` get a fitted candidate
  # back from the engine and must turn it into the same object this function
  # returns, without refitting it. See `.ferx_fit_from_raw()`.
  result <- .ferx_fit_from_raw(
    raw,
    model            = model,
    data             = data,
    gradient_arg     = gradient_arg,
    settings_used    = settings_used,
    model_file_opts  = model_file_opts,
